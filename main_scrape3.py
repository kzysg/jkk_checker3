...
HASH_FILE = "result_hash.txt"  # 追加：通知判定用のハッシュ保存ファイル
# 物件データ部の正規化
results_sorted = sorted(results, key=lambda d: (d.get("募集番号", ""), d.get("住宅コード", ""))) # 並び順安定化
data_for_hash = json.dumps(results_sorted, ensure_ascii=False, sort_keys=True, separators=(',',':')) # キー順+無駄スペース排除
